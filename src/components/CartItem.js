...
import { getBaseUrl, getPrice } from '../utils';
...
function CartItem({ item, setHasFetched }) {
  ...
  const deleteCartItem = (itemId) => {
    axios.delete(`https://eshop.reskimulud.my.id/products${getBaseUrl()}/carts/${itemId}`, {
      headers: {
  ...
  return (
    <Card key={item.id} className='mt-0 p-3 m-3 p-md-4'>
      <Row>
        <Col className='col-1'>
          <Card.Img style={imageStyle} variant="top" src={`https://eshop.reskimulud.my.id/products/image/${item.image}`} />        </Col>
